...
package com.gs.app.netsuiteerp.ui.pages.inventory_adjustments.respository
...
class InventoryAdjustmentsNetRepository {
    fun getInventoryAdjustments(token: OAuth2Token, q: String?, limit: Int, offset: Int): Call<PageData> {
        return if (TextUtils.isEmpty(q))
            if (limit > 0)
                RetrofitClient.api.getInventoryAdjustments(token.token_type + " " + token.access_token, "$limit", "$offset")
            else
                RetrofitClient.api.getInventoryAdjustments(token.token_type + " " + token.access_token)
        else
            if (limit > 0)
                RetrofitClient.api.getInventoryAdjustments(token.token_type + " " + token.access_token, q!!, "$limit", "$offset")
            else
                RetrofitClient.api.getInventoryAdjustments(token.token_type + " " + token.access_token, q!!)
    }

    fun getInventoryAdjustment(token: OAuth2Token, id: String): Call<InventoryAdjustment> {
        return RetrofitClient.api.getInventoryAdjustment(id, token.token_type + " " + token.access_token)
    }
